...
var accessFeature=['3d_city', 'game_choose', 'street_chat', 'who_likes_you', 'profile_visitors'];
...
function checkAccessFeature(el,action){
    var $el=$(el), feature='3d_city';
    if(action=='who_likes_you'){
        feature='encounters';
    }else if(action=='profile_visitors'){
        feature='profile_visitors_paid';
    }
...
function showConfirm(msg, handler, hCancel, btnOk, btnCancel, onlyOk, notClose, alingCenter, fa){
    ...
    if (onlyOk||0) {
        ...
        $('.navyblue', $pp).html(btnCancel).one('click',function(){
            ...
            $jq('#btn_send_like').click();
            closeAlert();
            return false;
        })
    }
    //$('.cont_pp_msg', $pp)[alingCenter?'addClass':'removeClass']('center');
    var pic=$pp.find('.pp_msg_pic')[0] || $pp.find('.bl_frm')[0];
    if(pic){
