var CMessages = function(guid, imHistoryMessages) {
    ...
    this.send = function(){
		...
		if(!text && !$this.isImageLoaded)return false;

		var send= +new Date,
			$msg=$jq('#template_msg').clone();

        if($this.isImageLoaded){
            $msg.find('.info').html('<div class="mod_image_preload">...</div>' + '\n');
        }

        var $node=$msg.find('.info');
        $node[0].innerHTML +=strToHtml(text);
        ...
        $.post(url_main+'messages.php?cmd=send_message&display=one_chat',
			  {ajax:1,user_to:userTo,msg:text,send:send,to_delete:0,
               image_upload:$this.isImageLoaded?1:0,ind:$this.isImageLoaded},
              function(res){
                var data=checkDataAjax(res);
                if(data!==false){
                    if(data=='redirect'){
	...
	this.showOriginalMessage=function($el) {
        $el.fadeOut(300);
        $el.parent('.pp_message_original_title').next('.pp_message_original_text')
        .slideDown({duration: 300, step:function(){
            $this.reInitToScrollPanelChat('relative')
        }})
    }
    ...
    this.initLightbox = function($img) {
        ...
        var data={
            ...
            imageLoading:  url_tmpl+'common/lightbox/images/loader.gif',
    ...
    this.initAddImage = function(){
        ...
        $('#pp_message_upload_img_frm').submit(function(e){
            var indx=+new Date,
                ...
                if (error) {
                    alertCustom(error, true, l('alert_html_alert'));
                    return false;
                }
                $this.processUploadImage=true;
                $this.addImageImLoader.addChildrenLoader();
                $this.addImageIm.addClass('to_hide').attr('title', '');
                        ...
                        if(xhr.status == 200) {
                            ...
                            if (data) {
                                if (data!='complete_upload_image') {
                                    alertCustom(data, true, l('alert_html_alert'));
                                } else {
                                    $this.isImageLoaded=indx;

                                    $('#pp_message_upload_img').removeClass('disabled')
                                                               .attr('title', l('upload_image_delete'));
                                }
                            }
                            $this.processUploadImage = false;
                            $this.addImageIm.removeClass('to_hide');
