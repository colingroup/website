var CMessages = function(guid, guidPhoto, imHistoryMessages) {
    ...
    this.changeActiveFirstChat = function($contact,noTrans){
        noTrans=noTrans||0;
        if(!$contact[0]){
            $this.$ppMsg.attr('placeholder', l('write_your_message'));
            $this.hideUserContent();
            return;
        }
    ...
    this.send = function(){
        ...
        var msgOrig=msg;
        if (preload) {
            msgHtml ='<div class="image_preload">' + preload + '</div>' + '\n' + msgHtml;
            if (!msgOrig) {
                msgOrig = preload;
            }
        } else {
            msgOrig=strToHtml(msg, true)
        }

        var $msg=$this.getTemplateMsg(send, msgHtml, strToHtml(msgOrig, true), audioMessageId, groupId, fromGroupId, toGroupId),
        ...
        var fnSend=function(retry){
            var data = {
                    ...
                    image_upload:$this.isImageLoaded?1:0,
                    ind:$this.isImageLoaded
                };
            retry=retry||0;
    ...
    this.updateMsgPreview = function($msg){
        ...
        if (!$msgPreview.find('.fa-picture-o, .fa-play-circle, i.image_preview')[0]) {
