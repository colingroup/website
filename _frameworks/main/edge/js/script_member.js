$(function(){
    ...
    $jq('.menu_invite_friends_by_sms_edge').click(function(){
        ...
        var message = 'Try out Yoolp! ' + $('base').attr('href');
        //alert(message);
        YoolpContacts.inviteFriends(message);
...
function updateGridPhoto(){
    ...
    $.post(pageUrl,{upload_only_header_ajax:1},function(res){
        var $page=$(res);
...
function runAppIosPhotoEditor(imageId, isGallery)
{
...
function runAppIosImAudioMessageRecorder()
{
    var debug = false;
        ...
        document.addEventListener(
            ...
            function() {
                navigator.device.capture.captureAudio(
                    ...
                    {
                        limit: 1,
                        successUrl: $('base').prop('href') + 'ajax.php?cmd=save_im_audio_message'
                    }
                )
            }
        );
