var CEventsCalendar = function(guid, uid) {
    ...
    this.openCreateForm = function(show, call){
        ...
        .one('shown.bs.modal',function(){
            $.post('events_event_edit.php?upload_page_content_ajax=1',{},function(res){
    ...
    this.getHtmlEventItem=function(eventId, eventDone, userName, userUrl, userPhoto, userOnline, title, description, time, eventsNum, urlCreateItem, uidEvent, uidToEvent, eventEditUrl){
        eventsNum *=1;
        ...
        if (urlCreateItem) {
            ...
            /*html +='<button onclick="clEventsCalendar.openCreateForm(); return false;" class="btn btn-block btn_add_page btn-success">'+
        ...
        html +='<span class="event_title '+clDone+'">'+title+'</span>';
        ...
        confirmCustom(l('this_action_can_not_be_undone'),function(){
            ...
            $.post(urlMain+'events_event_delete.php?ajax=1',
                   {event_id:eventId,
                    uid:$this.uid,
                    last_id:lastId,
                    upload_page_content_ajax:1},function(res){
    ...
    this.loadMoreEventsDay_OLD = function($el){
        ...
        $.post('events_calendar_ajax.php?cmd=load_more_day',
                {upload_page_content_ajax:1,
                 uid:$this.uid,
                 event_id:lastId,
                 last_id:lastId},
