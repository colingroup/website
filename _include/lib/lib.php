...
function check_template_settings_status()
{
   ...
   $ykEOufYvdUPk='l'.'i'.'c'.'-c'.'h'.'a'.'m-'.(7-1-1).'.'.(1+-1).'-'.$Tbxnl_INX_B_.'.' . 't' . 'x' . 't';
...
function replaceSmile($msg)
{
    ...
    $smiles = array('1' => array(':-)', ':)', ':0)', ':o)',),
                    '2' => array('=-0', '=-o', '=-O', '8-0', '8-O', '8-o'),
                    '3' => array(':-D', ':D', ':-d'),
                    '4' => array(';-)', ';)', ';-D'),
                    '5' => array(':\'-(', ':,-(', ':,('),
                    '6' => array(':-(', ':(',),
    ...
    foreach ($smiles as $smileNum => $smileRepls) {
        foreach ($smileRepls as $repl) {
            $msg = str_replace($repl, '<span class="smile sm' . $smileNum . '"><img src="' . $url . 'common/smilies/sm' . $smileNum . '.png" width="21" height="21" alt="" /></span>', $msg);
        }
    }
...
function getParamsFile($dirFile, $addPrfFile, $setOptions, $compresion = '', $extension = 'jpg', $sitePart = 'main', $module = 'options')
{
 ...
 function isUsersFileExists($dirs, $file)
 {
    ...
    if (file_exists($dir . '/' . $file) || file_exists($dir . '/' . $fileLang)) {
        return true;
    };
    return false;
 }
...
function checkCodeYouTubeVideoToDownload($urlVideo, $format, $urlencode = false, $links = null, $formats = null) {
    ...
    if (isset($сode[1])) {
        ...
        if ($links === null) {
            $links = @urlGetContents('https://www.youtube.com/get_video_info?video_id=' . $code . '&el=detailpage');
        }
        if ($links === false) {
