...
class Wall {
    ...
    static function add($section, $item = 0, $uid = false, $params = '', $unique = false, $hideFromUser = 0, $access = 'public', $parent = 0, $paramsSection = '', $groupId = null)
    {
    ...
    static function itemInfoPrepare($row)
    {
        ...
        if(isset($row['item_name'])) {
            ...
            $vars['share_photo'] = self::getPhotoUserItem($row, 'r', $isGroupPage ? $groupId : 0);
            ...
            $vars['photo'] = self::getPhotoUserItem($row, $sizePhoto, $isGroupPage ? $groupId : 0);
        ...
        $vars['wall_item_title_template'] = self::wallItemTitleTemplate($row['section'] . $prfTitle, $row['gender'], $isGroupPage ?  0 : $groupId);
    ...
    static function parseItems(&$html, $id = false, $oneOnly = false, $newOnly = false, $isNumberPostsUser = false, $groupId = null)
    {
        ...
        if ($optionTemplateSet == 'urban') {
            ...
            if (self::$typeWall == 'edge') {
                $whereInterests = ' OR w.section = "share" OR w.section = "group_social_created" '
                                . ' OR w.section = "photo_default" OR w.section = "blog_post"';
            } else {
        ...
        if ($optionTemplateSet == 'urban') {//Доступна только для авториз
            if ($isWallInProfile) {
                ...
                $whereUser = ' w.user_id = ' . to_sql($uid, 'Number');
                if (self::$typeWall == 'edge') {
                    if (!$groupId && $groupsSubscribersListWallUid) {
                        $whereUser = ' (
                                         (w.user_id = ' . to_sql($uid, 'Number') . ' AND w.group_id = 0)
                                      OR (
                                           w.group_id != 0 AND
                                           (
                ...
                $where = $whereFriends . $whereUser . $where . $whereWallSql;
            }
        } else {
            $whereFriends = ' AND ' . $whereFriends;
            if ($uid != $guid) {
        ...
        while ($row = DB::fetch_row(3)) {
            ...
            if (self::$typeWall == 'edge') {
                ...
                if ($row['section_real'] == 'share') {
                    ...
                    $vars['wall_item_title'] = self::wallItemTitleTemplate('add_video' . $prfTitle, $row['gender']);
                ...
                } elseif (isset($vars['photo_count'])) {//Upload photos
