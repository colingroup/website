...
class Blogs extends CHtmlBlock
{
	...
	{
        $text = preg_replace_callback('%(<a[^>]href=["|\']{1}([^"|\']+)["|\']{1}>([^<]+)</a>)%', 'self::callSanitize', $text);
    ...
    static function addComment($isNotifComments = true)
    {
        ...
        if ($blogId && $msg) {
            ...
            $sql = "INSERT INTO `blogs_comment` (`id`, `user_id`, `post_id`, `post_user_id`, `dt`, `text`, `parent_id`, `parent_user_id`, `is_new`, `send`)
    ...
    static function deleteComment($commentId = 0, $isAdmin = false)
    {
        ...
        if ($guid == $comment['user_id'] || $guid == $comment['post_user_id']) {
            ...
            DB::update('user', array('blog_comments' => 'blog_comments-1'), 'user_id = ' . to_sql($comment['post_user_id']), '', '', true);
    ...
    static public function getImg($blogId, $imgId, $size)
    {
        ...
        if (!file_exists($g['path']['url_files'] . $file)) {
            $file = '';
        }
