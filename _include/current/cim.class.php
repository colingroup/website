            ...
            if ($notMsgToDb) {
                ...
                if ($imSendImage) {
                    if ($imageUpload) {
                        ...
                        $msg = OutsideImages::filter_to_db($msg, null, OutsideImages::$sizesIm);
	...
	static function parseImOneMsg(&$html, $row, $js = false, $update = 0, $isFbMode = 'false', $listUsersOpen = array())
    {
		...
		if (!empty($row) && is_array($row)) {
            ...
            if(isset($row['audio_message_id']) && $row['audio_message_id']) {
                ...
                $html->setvar($blockMsg . $blockMsgType . '_audio_message_file', ImAudioMessage::getUrl($row['audio_message_id']));
